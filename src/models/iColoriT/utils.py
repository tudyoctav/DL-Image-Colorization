...
def rgb_to_lms(rgb_tensor):
    # Define the RGB to LMS conversion matrix
    conversion_matrix1 = torch.tensor([[0.4124564, 0.3575761, 0.1804375],
                  [0.2126729, 0.7151522, 0.0721750],
                  [0.0193339, 0.1191920, 0.9503041]]).to(rgb_tensor.get_device())
    conversion_matrix2 = torch.tensor([[ 0.4002, 0.7076, -0.0808],
                 [-0.2263, 1.1653,  0.0457],
                 [      0,      0,  0.9182]]).to(rgb_tensor.get_device())
    conversion_matrix = torch.matmul(conversion_matrix1, conversion_matrix2)
    # Reshape the tensor to [B * W * H, C]
    reshaped_tensor = rgb_tensor.reshape(-1, rgb_tensor.shape[1])

    # Perform the RGB to LMS conversion
    lms_tensor = torch.matmul(reshaped_tensor, conversion_matrix)
    lms_tensor[lms_tensor > 1] = 1
    # Reshape the tensor back to [B, W, H, C]
    lms_tensor = lms_tensor.reshape(rgb_tensor.shape)

    return lms_tensor


def lms_to_rgb(lms_tensor):
    # Define the LMS to RGB conversion matrix
    conversion_matrix1 = torch.tensor([[0.4124564, 0.3575761, 0.1804375],
                  [0.2126729, 0.7151522, 0.0721750],
                  [0.0193339, 0.1191920, 0.9503041]]).to(lms_tensor.get_device())
    conversion_matrix2 = torch.tensor([[ 0.4002, 0.7076, -0.0808],
                 [-0.2263, 1.1653,  0.0457],
                 [      0,      0,  0.9182]]).to(lms_tensor.get_device())
    inverse_conversion_matrix = torch.linalg.inv(torch.matmul(conversion_matrix1, conversion_matrix2))
    # Reshape the tensor to [B * W * H, C]
    reshaped_tensor = lms_tensor.reshape(-1, lms_tensor.shape[1])

    # Perform the LMS to RGB conversion
    rgb_tensor = torch.matmul(reshaped_tensor, inverse_conversion_matrix)
    rgb_tensor[rgb_tensor>1] = 1
    # Reshape the tensor back to [B, W, H, C]
    rgb_tensor = rgb_tensor.reshape(lms_tensor.shape)
