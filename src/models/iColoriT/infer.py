...
from utils import lab2rgb, psnr, rgb2lab, seed_worker, rgb_to_lms, lms_to_rgb
import PIL
...
def get_args():
    ...
    # For evaluation
    parser.add_argument('--model_path', type=str, help='checkpoint path of model', default='iColoriT_demo/iColoriT.pth')
    parser.add_argument('--model_args_path', type=str, help='args.pkl path of model', default='')
    parser.add_argument('--val_data_path', default='data', type=str, help='validation dataset path')
    parser.add_argument('--val_hint_dir', type=str, help='hint directory for fixed validation', default='data/hints/1234')
    parser.add_argument('--pred_dir', type=str, default='results', help='save all prediction here')
    ...
    parser.add_argument('--val_hint_list', default=[0, 1, 2, 5, 50], nargs='+')
    #Colorblindness
    parser.add_argument('--colorblind_strategy', type=str, default='NONE', help='Strategy to compute colorblind images: you can use \'AVG\', \'ZERO\' or \'NONE\'')
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(data_loader_val):
            ...
            for i, count in enumerate(args.val_hint_list):
                ...
                bool_hint_r = torch.reshape(bool_hint, (bool_hint.shape[0], 224, 224)).unsqueeze(dim=1)
                _img_mask = np.concatenate((images_lab[:,1:3,:,:].clone().cpu().detach().numpy() / 110, (255 - bool_hint_r.clone().cpu().detach().numpy()) / 255), axis=1)
                _img_mask = torch.from_numpy(_img_mask).type(torch.FloatTensor).to(device)
                ...
                pred_imgs_lab = rearrange(outputs, 'b (h w) (p1 p2) c -> b c (h p1) (w p2)',
                ...
                if args.pred_dir is not None:
                    ...
                    for name, pred_img in zip(names, pred_imgs_lab):
                        im = cv2.imread((os.path.join(args.val_data_path, 'imgs', name )))
                        img = pred_img.cpu().detach().numpy()
                        img = np.transpose(img, (1, 2, 0))
                        img = cv2.resize(np.float32(img), (im.shape[1], im.shape[0]), interpolation=cv2.INTER_CUBIC)
                        img = img * 110

                        targ = color.rgb2lab(im[:, :, ::-1])[:, :, 0]


                        img = np.concatenate((targ[..., np.newaxis], img), axis=2)
                        img = (np.clip(color.lab2rgb(img), 0, 1) * 255).astype('uint8')
                        img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                        cv2.imwrite(osp.join(img_save_dir, osp.splitext(name)[0] + '.png'), img)
